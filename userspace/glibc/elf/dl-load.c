...
// BEGIN DOPLNENE
static const char origin_based_dirs[] = ORIGIN_BASED_DIRS;
static const size_t origin_based_dirs_len[] =
{
  ORIGIN_BASED_DIRS_LEN
};
#define norigin_based_search_dirs_len array_length (origin_based_dirs_len)
// END DOPLNENE
...
struct r_search_path_struct __origin_based_search_dirs attribute_relro; // DOPLNENE
		...
		const char *glibc_hwcaps_mask)
{
  ...
  //max_dirnamelen = SYSTEM_DIRS_MAX_LEN; // ZAPOZNAMKOVANE
  //*aelem = NULL; // ZAPOZNAMKOVANE
  ...
  // BEGIN DOPLNENE
  /* Next set up the rest of the origin based search directory entries.  */
  if ((l)->l_origin == NULL){
    assert ((l)->l_name[0] == '\0' || is_rtld_link_map (l));
    (l)->l_origin = _dl_get_origin();
  }

  if(l->l_origin && l->l_origin != (char *)-1){
    origin = (char *)l->l_origin;
    l_origin_len = strlen(origin);
  }

  if(origin){
      aelem = __origin_based_search_dirs.dirs = (struct r_search_path_elem **) malloc ((norigin_based_search_dirs_len + 1) * sizeof (struct r_search_path_elem *));
      if (__origin_based_search_dirs.dirs == NULL) {
          errstring = N_("cannot create origin based search path array");
      signal2_error:
          _dl_signal_error (ENOMEM, NULL, NULL, errstring);
      }

      round_size = ((2 * sizeof (struct r_search_path_elem) - 1 + ncapstr * sizeof (enum r_dir_status)) / sizeof (struct r_search_path_elem));
      __origin_based_search_dirs.dirs[0] = malloc (norigin_based_search_dirs_len * round_size * sizeof (*__origin_based_search_dirs.dirs[0]));
      if (__origin_based_search_dirs.dirs[0] == NULL){
        errstring = N_("cannot create cache for origin based search path");
        goto signal2_error;
      }

      __origin_based_search_dirs.malloced = 0;
      pelem = GL(dl_all_dirs) = __origin_based_search_dirs.dirs[0]; // FIXNUT ASI
      strp = origin_based_dirs;
      idx = 0;
      ...
      do
        {
          ...
          *aelem++ = pelem;
          ...
          pelem->what = "origin based search path";
          pelem->where = NULL;

          char *resulting_path = malloc((strlen(strp) + l_origin_len - 5) * sizeof(char));
          _dl_dst_substitute(l, strp, resulting_path);

          pelem->dirname = resulting_path;
          pelem->dirnamelen = strlen(resulting_path);

          strp += origin_based_dirs_len[idx] + 1;
          ...
          pelem->next = (++idx == norigin_based_search_dirs_len ? NULL : (pelem + round_size));

          pelem += round_size;
        }
      while (idx < norigin_based_search_dirs_len);

    // END DOPLNENE

      max_dirnamelen = SYSTEM_DIRS_MAX_LEN > ORIGIN_BASED_DIRS_MAX_LEN ? SYSTEM_DIRS_MAX_LEN : ORIGIN_BASED_DIRS_MAX_LEN; // ZMENENE
  }else{
...
#endif
	  /* Before default dirs, try origin based search dirs.  */
	  if(origin){
          if (fd == -1
          && ((l = loader ?: GL(dl_ns)[nsid]._ns_loaded) == NULL
              || __glibc_likely (!(l->l_flags_1 & DF_1_NODEFLIB)))
          && __origin_based_search_dirs.dirs != (void *) -1)
        fd = open_path (name, namelen, mode, &__origin_based_search_dirs,
